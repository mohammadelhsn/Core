...
export default class TattooCommand extends BaseCommand {
	constructor() {
		super(
			'tattoo',
			'canvas',
	...
	async run(client: DiscordClient, message: Message, args: string[]) {
		...
		if (mention) {
			...
			try {
				...
				const image = await new Tatoo().getImage(avatar);
				const file = new MessageAttachment(image, 'tattoo.png');

				const embed = await this.ImageEmbed.Base({
					iconURL: message.author.displayAvatarURL({ dynamic: true }),
					text: this,
					title: 'Tattoo command',
					description: guild.Strings.DiscordIG,
					image: 'attachment://tattoo.png',
				});
