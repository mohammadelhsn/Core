...
export default class DeleteTrashCommand extends BaseCommand {
	constructor() {
		super(
			'deletetrash',
			'canvas',
	...
	async run(client: DiscordClient, message: Message, args: string[]) {
		...
		if (mention) {
			...
			try {
				...
				const file = new MessageAttachment(image, 'delete_trash.png');

				const embed = await this.ImageEmbed.Base({
					iconURL: message.author.displayAvatarURL({ dynamic: true }),
					text: this,
					title: 'DeleteTrash command',
					description: guild.Strings.DiscordIG,
					image: 'attachment://delete_trash.png',
				});
