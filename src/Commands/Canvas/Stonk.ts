...
export default class StonkCommand extends BaseCommand {
	constructor() {
		super(
			'stonk',
			'canvas',
	...
	async run(client: DiscordClient, message: Message, args: string[]) {
		...
		if (mention) {
			...
			try {
				...
				const image = await new Stonk().getImage(avatar);
				const file = new MessageAttachment(image, 'stonk.png');
