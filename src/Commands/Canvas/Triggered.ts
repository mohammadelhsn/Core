...
export default class TriggeredCommand extends BaseCommand {
	constructor() {
		super(
			'triggered',
			'canvas',
	...
	async run(client: DiscordClient, message: Message, args: string[]) {
		...
		if (mention) {
			...
			try {
				...
				const image = await this.Canvas.Triggered(avatar);
				const file = new MessageAttachment(image.file, 'triggered.png');

				const embed = await this.ImageEmbed.Base({
					iconURL: message.author.displayAvatarURL({ dynamic: true }),
					text: this,
					title: 'Triggered command',
					description: guild.Strings.DiscordIG,
					image: 'attachment://triggered.png',
				});
