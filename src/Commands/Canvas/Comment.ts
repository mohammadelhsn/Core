...
export default class CommentCommand extends BaseCommand {
	constructor() {
		super(
			'comment',
			'canvas',
		...
		if (firstColl.size > 0) {
			...
			if (
				...
				if (
					...
					const image = await this.Canvas.Comment(avatar, title, content);
					const file = new MessageAttachment(image.file, 'comment.png');
