...
export default class ConfusedStonkCommand extends BaseCommand {
	constructor() {
		super(
			'confusedstonk',
			'canvas',
	...
	async run(client: DiscordClient, message: Message, args: string[]) {
		...
		if (mention) {
			...
			try {
				...
				const file = new MessageAttachment(image, 'confused_stonk.png');

				const embed = await this.ImageEmbed.Base({
					iconURL: message.author.displayAvatarURL({ dynamic: true }),
					text: this,
					title: 'ConfusedStonk command',
					description: guild.Strings.DiscordIG,
					image: 'attachment://confused_stonk.png',
				});
