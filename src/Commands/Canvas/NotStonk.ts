...
export default class NotstonkCommand extends BaseCommand {
	constructor() {
		super(
			'notstonk',
			'canvas',
	...
	async run(client: DiscordClient, message: Message, args: string[]) {
		...
		if (mention) {
			...
			try {
				...
				const file = new MessageAttachment(image, 'not_stonk.png');

				const embed = await this.ImageEmbed.Base({
					iconURL: message.author.displayAvatarURL({ dynamic: true }),
					text: this,
					title: 'NotStonk command',
					description: this.Utils.FormatProvider('Discord IG'),
					image: 'attachment://not_stonk.png',
				});
