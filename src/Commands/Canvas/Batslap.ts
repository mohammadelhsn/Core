...
export default class BatslapCommand extends BaseCommand {
	...
	async run(client: DiscordClient, message: Message, args: string[]) {
		...
		if (message.mentions.members.size == 2) {
			...
			try {
				const image = await new Batslap().getImage(user1, user2);
				const file = new MessageAttachment(image, 'batslap.png');
		...
		if (message.mentions.members.size == 1) {
			...
			try {
				const image = await new Batslap().getImage(user2, user1);
				const file = new MessageAttachment(image, 'batslap.png');
