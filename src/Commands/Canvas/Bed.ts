...
export default class BedCommand extends BaseCommand {
	constructor() {
		super(
			'bed',
			'canvas',
	...
	async run(client: DiscordClient, message: Message, args: string[]) {
		...
		if (message.mentions.members.size == 2) {
			...
			try {
				const image = await new Bed().getImage(user1, user2);
				const file = new MessageAttachment(image, 'bed.png');
		...
		if (message.mentions.members.size == 1) {
			...
			try {
				const image = await new Bed().getImage(user2, user1);
				const file = new MessageAttachment(image, 'bed.png');
		...
		if (firstColl.size > 0) {
			...
			if (secondColl.size > 0) {
				...
				const m = await message.channel.send({ embeds: [gEmbed] });
				try {
					const image = await new Bed().getImage(attach, attach2);
